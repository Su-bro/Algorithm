...
//        PriorityQueue<Info> pq = new PriorityQueue<Info>();
        Queue<Info> pq = new LinkedList<Info>();
        //오른쪽으로 갈수있으면, 아래로 내려갈수있으면 직선가격, 방향 추가해준다
        if(board[0][1]!=1)pq.add(new Info(0,1,100,1));
        if(board[1][0]!=1)pq.add(new Info(1,0,100,0));
...
//        	if(y==N-1 && x==N-1) return now.price; //도착하면 가격리턴해주자
        	for (int d = 0; d < 4; d++) {
				...
				if(memo[ny][nx] >= price+nextprice) { //해당메모이제이션의 가격보다 쌀 경우
					...
					pq.add(new Info(ny, nx, price+nextprice, d)); //도로가격더한거 add
				}
			}        	
        }        
        return memo[N-1][N-1];
